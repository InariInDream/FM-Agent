...
4. `.spec.json` supports optional property fields: `invariants` (properties that must hold on every loop iteration or event-handling cycle of non-terminating/long-running functions), `resources` (acquire/release and leak-freedom contracts for memory, locks, handles, connections), and `ordering` (required ordering between named operations/events). For each declared field, the reasoner additionally verifies that every code block satisfies it at all times while running.
